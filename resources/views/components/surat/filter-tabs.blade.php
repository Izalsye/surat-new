@props([
    'selectedTab' => 'inbox', // Default value
    'categories' => [ // Sample categories data
        ['name' => 'Personal', 'color' => 'primary', 'value' => 'personal'],
        ['name' => 'Work', 'color' => 'warning', 'value' => 'work'],
        ['name' => 'Social', 'color' => 'success', 'value' => 'social'],
        ['name' => 'Private', 'color' => 'danger', 'value' => 'private'],
    ],
    'pager' => null, // Pass pager data for pagination controls
...
<div class="flex flex-wrap flex-col md:flex-row xl:w-auto justify-between items-center px-4 pb-4">
    <div class="w-full sm:w-auto grid grid-cols-2 sm:grid-cols-4 gap-3 mt-4">
        @foreach ($categories as $category)
            <button type="button"
                class="btn btn-outline-{{ $category['color'] }} flex"
                :class="{ 'text-white bg-{{ $category['color'] }}': selectedTab === '{{ $category['value'] }}' }"
                @click="$dispatch('tabchanged', '{{ $category['value'] }}')">

                <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                    xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 ltr:mr-2 rtl:ml-2">
                    {{-- Conditional SVG based on category can be added here if needed --}}
                    @if ($category['value'] === 'personal')
                        <circle cx="12" cy="6" r="4" stroke="currentColor"
                            stroke-width="1.5" />
                        <ellipse opacity="0.5" cx="12" cy="17" rx="7"
                            ry="4" stroke="currentColor" stroke-width="1.5" />
                    @elseif ($category['value'] === 'work')
                        <path
    ...
    {{-- Pagination was here, moved to search-pagination component --}}
</div>
