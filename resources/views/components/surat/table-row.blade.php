@props(['pagedMails', 'selectedTab', 'ids' => []])

<template x-if="pagedMails.length">
    <div class="table-responsive grow overflow-y-auto min-h-[280px]">
        <table>
            <tbody>
                <template x-for="mail in pagedMails" :key="mail.id + '' + index">
                    <tr class="cursor-pointer " @click="selectMail(mail)">
                        <td>
                            <div class="flex items-center whitespace-nowrap ">
                                <div class="ltr:mr-3 rtl:ml-3">
                                    <input type="checkbox" :id="`chk-${mail.id}`" x-model.number="ids"
                                        :value="(mail.id)" @click="$event.stopPropagation()" class="form-checkbox" />
                                </div>
                                <div class="ltr:mr-3 rtl:ml-3" x-tooltip="Star" data-placement="top" role="tooltip">
                                    <button type="button"
                                        ...
                                        :class="{ 'text-warning': mail.isStar }" @click.stop="setStar(mail.id)"
                                ...
                                <div class="ltr:mr-3 rtl:ml-3" x-tooltip="Important" data-placement="top"
                                    role="tooltip">
                                    <button type="button"
                                        class="enabled:hover:text-primary disabled:opacity-60 rotate-90 flex items-center"
                                        :class="{ 'text-primary': mail.isImportant }"
                                        @click.stop="setImportant(mail.id)" :disabled="selectedTab === 'trash'">
                                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                        ...
                        <td class="whitespace-nowrap font-medium ltr:text-right rtl:text-left" x-text="showTime(mail)">
                        </td>
                    </tr>
                </template>
            </tbody>
