            ...
            <div class="panel p-0 flex-1 overflow-auto h-full">
                <div x-show="!selectedMail && !isEdit" class="flex flex-col h-full">
                    <x-surat.toolbar
                        ...
                        :check-all-checkbox="checkAllCheckbox()"
                        :ids="ids"
                        @checkall="checkAll($event.detail)"
                        @refreshmails="refreshMails()"
                        @setarchive="setArchive()"
                        @setspam="setSpam()"
                        @setgroup="setGroup($event.detail)"
                        @setaction="setAction($event.detail)"
                        @togglemailmenu="isShowMailMenu = !isShowMailMenu"
                        {{-- Removed @searchmailskeyup as it's now handled by search-pagination --}}
                    />
                    <div class="h-px border-b border-[#e0e6ed] dark:border-[#1b2e4b]"></div>
                    <x-surat.search-pagination
                        :search-text="searchText"
                        :pager="pager"
                        :filtered-mail-list-count="filteredMailList.length"
                        @searchmailskeyup="searchText = $event.detail; searchMails()"
                    ...
                    <x-surat.filter-tabs
                        ...
                        :pager="pager"
                        :filtered-mail-list-count="filteredMailList.length"
                        @tabchanged="tabChanged($event.detail)"
                        @changepage="pager.currentPage = $event.detail; searchMails(false)"
                    />
                    <div class="h-px border-b border-[#e0e6ed] dark:border-[#1b2e4b]"></div>
                                ...
                                <tbody>
                                    <template x-for="mail in pagedMails" :key="mail.id + '' + index">
                                        <x-surat.table-row
                                            ...
                                            :selected-tab="selectedTab"
                                            :ids="ids"
                                            @selectmail="selectMail($event.detail)"
                                            @setstar="setStar($event.detail)"
                                            @setimportant="setImportant($event.detail)"
                                        />
                                    </template>
                                </tbody>
                            </table>
